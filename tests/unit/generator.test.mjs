...
import { REAL_PLAYERS, REAL_PLAYERS_EXTRA, REAL_PLAYERS_WAVE3, REAL_PLAYERS_WAVE4 } from '../../js/data/realPlayers.js';
...
test('world shape', () => {
  assert.equal(WORLD.clubs.length, 60);
  assert.equal(WORLD.players.length, 3092);
  assert.equal(WORLD.sbcCards.length, 12);
  assert.equal(new Set(WORLD.clubs.map((c) => c.league)).size, 6);
  for (const d of [1, 2, 3, 4, 5, 6]) assert.equal(WORLD.clubs.filter((c) => c.division === d).length, 10, `division ${d} has ten clubs`);
  // v70 world: the first 2052 cards are exactly what they were
  const v70 = WORLD.players.slice(0, 2052);
  assert.ok(v70.every((p) => !p.clubId || Number(p.clubId.slice(1)) <= 40), 'no pre-v71 card was moved to a new club');
...
test('every card is a real person with a flag', () => {
  const real = new Set([...REAL_PLAYERS, ...REAL_PLAYERS_EXTRA, ...REAL_PLAYERS_WAVE3, ...REAL_PLAYERS_WAVE4].map((r) => r[0]));
