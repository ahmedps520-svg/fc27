(() => {
  ...
  ], LEAGUE_NAME = "Apex Premier Division", LEAGUES = [LEAGUE_NAME, "Meridian League", "Vanguard League", "Foundation League"], POSITIONS = {
  ...
  function buildWorld() {
    ...
    CORE.forEach((bp, index) => {
      ...
      clubs.push({
        ...
        budget: Math.round((12 - bp.tier) * (bp.wave === 3 ? 2e6 : 5e6) + (bp.wave === 3 ? 25e5 : 6e6))
      });
    });
    let freeAgents = [];
    for (let i = 0; i < 34; i++) {
    ...
    for (let club of clubs) {
      if (!((_a = CLUB_BLUEPRINTS.find((bp) => bp.name === club.name)) != null && _a.wave)) continue;
